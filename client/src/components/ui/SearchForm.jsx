...
export default function SearchForm({ cards = [], setCards }) {
  ...
  const applyFilters = () => {
    ...
    if (filters.minPrice) {
      filteredCards = filteredCards.filter(
        (card) => Number(card?.price || 0) >= Number(filters.minPrice)
      );
    }

    if (filters.maxPrice) {
      filteredCards = filteredCards.filter(
        (card) => Number(card?.price || Infinity) <= Number(filters.maxPrice)
      );
    }
  ...
  filteredCards = filteredCards.filter(
    (card) =>
      card?.seller?.city?.toLowerCase().includes(filters.city.toLowerCase())
  );
}
      ...
      >
        ...
        <div className="filter-group">
          ...
          >
            ...
            <option value="Поврежденная">Поврежденная</option>
